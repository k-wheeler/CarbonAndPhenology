...
#for(l in 1:length(leafNames)){
  ...
  files <- dir(path=dataDirectory,pattern="-B")
  jmax <- numeric()
  vcmax <- numeric()
  ...
  if(length(files)>0){
    for(f in 1:length(files)){
      ...
      save(out,file=paste0('LicorFits/',leafNames[l],"_",dte,"_LicorResponseCurve_varBurn.RData"))
      print(paste("Saved:",paste0('LicorFits/',leafNames[l],"_",dte,"_LicorResponseCurve_varBurn.RData")))
